...
digitalWrite(8, HIGH);
 ...
 pinMode(12, OUTPUT);  // pin 4 como salida para controlar pololu 3
 pinMode(13, OUTPUT);   // pin 7 como salida para controlar pololu 3
...
void loop(){ 
 ...
 if(digitalRead(menuPin)==LOW )   //Cambiamos el modo a MHZ de ajuste a amateur y a broadcast
              { 
                funcionmenu++;
                if(funcionmenu>=7) funcionmenu = 0; 
                switch(funcionmenu){
...
if(millis()- Time2>= 5000 && (funcionmenu==1 || funcionmenu>=4)) //temporizador de cada ,5 segundos para antena
{
  Time2 = millis();
  pantalla();  
