...
void loop(){ 
  ...
  {
  //EEPROM.get(0,Eepromread);  //lee la posicion 0 de la memoria y la pone como posicion inicial de encoderValue
 ...
 if(digitalRead(memUP)==LOW || digitalRead(memDOWN)==LOW || savmem==1){
  ...
  if(digitalRead(memUP)==LOW || digitalRead(memDOWN)==LOW || savmem==1){  //Seleccionamos botones memoria arriba y abajo
    ...
    if(digitalRead(memDOWN)==LOW && funcionmenu == 1){
                 ...
                 switch(memstep){
                  ...
                  lcd.print("SAVE1  "); 
                  lcd.setCursor(12, 1);      // ubica cursor en columna 12 y linea 0
                  lcd.print(b);                                  
                  delay(3000); 
                  funcionmenu = 0;
    ...
    if (encoderValue < b) {  //Comparamos encodervalue con el paso de destino que en este caso es superior en numero
    ...
    for (encoderValue=encoderValue; encoderValue < b; encoderValue++) //bucle de pasos hasta que encodervalue y b se igualan
    {       
    digitalWrite(pololu, HIGH);       // nivel alto  
    delay(2);                            
    digitalWrite(pololu, LOW);        // nivel bajo  
    if(digitalRead(DOWN)==LOW) b = encoderValue;          
    }
    delay(500);
    lcd.setCursor(9,1);      // ubica cursor en columna 6 y linea 0
  ...
  if (encoderValue > b){  //Comparamos encodervalue con el paso de destino que en este caso es inferior en numero
    ...
    digitalWrite(desabilitar, LOW); 
    digitalWrite(direccion, LOW);        // giro en un sentido positivo   
    for (encoderValue=encoderValue; encoderValue > b; encoderValue--) //bucle de pasos hasta que encodervalue y b se igualan
    {    
    digitalWrite(pololu, HIGH);       // nivel alto  
    delay(2);                            
 ...
 if(digitalRead(menuPin)==LOW )   //Cambiamos el modo a MHZ de ajuste a amateur y a broadcast
              { 
                funcionmenu++;
                if(funcionmenu>=4) funcionmenu = 0;
                switch(funcionmenu){
 ...
 if(millis() - Time>= 2000) //temporizador de cada ,2 segundos
  {
  Time = millis();
